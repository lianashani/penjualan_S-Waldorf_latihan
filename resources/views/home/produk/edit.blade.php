    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h6 class="card-subtitle mb-4">Ubah data produk dengan varian</h6>

                <form action="{{ route('produk.update', $produk->id_produk) }}" method="POST" enctype="multipart/form-data" id="productForm">
                    @csrf
                    @method('PUT')
                                    ...
                                    <div class="col-6">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="is_featured" value="1"
                                                   {{ old('is_featured', $produk->is_featured) ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            ...
                            <div class="form-group">
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="has_variants" value="0" id="singleProduct"
                                           {{ old('has_variants', $produk->has_variants ? '1' : '0') == '0' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="singleProduct">Produk Tunggal</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="has_variants" value="1" id="variantProduct"
                                           {{ old('has_variants', $produk->has_variants ? '1' : '0') == '1' ? 'checked' : '' }}>
                    ...
                    <!-- Single Product Form -->
                    <div id="singleProductForm" style="display: {{ old('has_variants', $produk->has_variants ? '1' : '0') == '0' ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Ukuran <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control @error('ukuran') is-invalid @enderror"
                                           name="ukuran" value="{{ old('ukuran', $produk->ukuran) }}" placeholder="S, M, L, XL">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('warna') is-invalid @enderror"
                                           name="warna" value="{{ old('warna', $produk->warna) }}">
                    ...
                    <div id="variantProductForm" style="display: {{ old('has_variants', $produk->has_variants ? '1' : '0') == '1' ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-12">
                                <h6>Varian Produk</h6>
                                <div id="variantsContainer">
                                    @if($produk->has_variants && $produk->variants->count() > 0)
                                        @foreach($produk->variants as $index => $variant)
                                        <div class="variant-item border rounded p-3 mb-3" data-variant="{{ $index + 1 }}">
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                <h6 class="mb-0">Varian {{ $index + 1 }}</h6>
                                            ...
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label>Ukuran <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control" name="variants[{{ $index + 1 }}][ukuran]"
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label>Warna <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control" name="variants[{{ $index + 1 }}][warna]"
                                                ...
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="color" class="form-control form-control-color"
                                                               name="variants[{{ $index + 1 }}][kode_warna]" value="{{ $variant->kode_warna }}">
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        <label>Stok <span class="text-danger">*</span></label>
                                                        <input type="number" class="form-control" name="variants[{{ $index + 1 }}][stok]"
                                                ...
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        <label>Harga</label>
                                                        <input type="number" class="form-control" name="variants[{{ $index + 1 }}][harga]"
                                                               value="{{ $variant->harga }}" min="0" step="0.01">
                                                        <small class="text-muted">Kosongkan untuk menggunakan harga dasar</small>
                                                    </div>
                                                </div>
                                            </div>
                                            <input type="hidden" name="variants[{{ $index + 1 }}][id_variant]" value="{{ $variant->id_variant }}">
                                        </div>
                                        @endforeach
                                    @endif
                                </div>
                                        ...
                                        <div class="col-md-2 mb-2">
                                            <div class="position-relative">
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0"
                                                        onclick="removeExistingImage({{ $image->id_image }})" title="Hapus">
                                                    <i class="mdi mdi-close"></i>
                                                </button>
                                                @if($image->is_primary)
                                                    <span class="badge bg-primary position-absolute bottom-0 start-0">Utama</span>
                                                @endif
                                            </div>
                                        </div>
                                        @endforeach
                                ...
                                <div class="form-group">
                                    <div class="file-upload-area border-2 border-dashed border-primary rounded p-4 text-center">
                                        ...
                                        <div class="upload-content">
                                            ...
                                            <h5>Drag & Drop gambar baru di sini</h5>
...
$(document).ready(function() {
    let variantCount = {{ $produk->variants->count() }};
    let selectedImages = [];
...
function removeExistingImage(imageId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const deleteInput = `<input type="hidden" name="delete_images[]" value="${imageId}">`;
            $('#productForm').append(deleteInput);

            // Hide the image container
            $(`button[onclick="removeExistingImage(${imageId})"]`).closest('.col-md-2').hide();

            Swal.fire('Dihapus!', 'Gambar akan dihapus saat form disimpan', 'success');
        }
    });
}
</script>
